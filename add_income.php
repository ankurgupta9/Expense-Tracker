...
<head>
    <title>Expense Tracker - Add Income</title>
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav ms-auto">
                        ...
                        <li class="nav-item mx-2">
                            <a class="nav-link text-primary" href="add_income.php"><svg
                        ...
                        <li class="nav-item mx-2">
                            <a class="nav-link" href="add_expense.php"><svg
                                    xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
            ...
            <div align="left">
                ...
                <form action="add_income.php" method="post">
                    ...
                    <div class="form-group p-2">
                        ...
                        <input type="number" class="form-control" name="incomeAmount" id="incomeAmount" placeholder="0"
                            required>
                    </div>
                    <div class="form-group p-2">
                        <label for="incomeDesc">Description:</label>
                        <input type="text" class="form-control" name="incomeDesc" id="incomeDesc"
                            placeholder="About income" maxlength="50">
                    </div>
                    <?php
                    if (isset($_POST["submit"])) {
                        $userID = $_SESSION["UserID"];
                        $incAmt = $_POST["incomeAmount"];
                        $incDesc = $_POST["incomeDesc"];
                        $incDate = $_POST["incomeDate"];
                        
                        // Convert the date to MySQL DATE format
                        $dateTime = new DateTime($incDate);
                        $incDate = $dateTime->format('Y-m-d');

                        $sql = "INSERT INTO income (User_ID, inc_date, inc_amount, inc_desc) VALUES (?,?,?,?)";
                        $stmt = mysqli_stmt_init($conn);
                        $prepareStmt = mysqli_stmt_prepare($stmt, $sql);
                        
                        if ($prepareStmt) {
                            mysqli_stmt_bind_param($stmt, "isis", $userID, $incDate, $incAmt, $incDesc);
                            ...
                            if (mysqli_stmt_prepare($stmtUpdateRemInc, $updateRemIncSql)) {
                                mysqli_stmt_bind_param($stmtUpdateRemInc, "ii", $incAmt, $userID);
                    ...
                    <div align="center" class="mt-3">
                        <button type="submit" name="submit" id="btn" value="addinc" class="btn btn-primary px-5">Add
                            Income</button>
                    </div>
                </form>
            </div>
